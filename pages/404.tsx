...
export default function Custom404() {
  ...
  const pathWithoutBase = asPath === '/oux' ? '/' : asPath.replace(/^\/oux(?=\/)/, '');
  const isHant = pathWithoutBase.startsWith('/zh-hant');

  const title = isHant ? '404 - 頁面不存在 | 歐易下載入口' : '404 - 页面不存在 | 欧易下载入口';
  const desc = isHant
    ? '您訪問的頁面不存在，請返回歐易下載入口繼續瀏覽。'
    : '您访问的页面不存在，请返回欧易下载入口继续浏览。';
